...
func parseAgencyVehicleRow(row []string, reportMonth string, reportYear int) *AgencyVehicleRecord {
	...
	if record.EndingMiles > 0 && record.BeginningMiles > 0 {
		if record.EndingMiles < record.BeginningMiles {
			log.Printf("Warning: Ending miles (%d) less than beginning miles (%d) for vehicle %s", 
				record.EndingMiles, record.BeginningMiles, record.VehicleID)
			// Calculate correct total miles
			record.TotalMiles = 0 // Set to 0 to indicate invalid data
		} else {
			// Calculate expected total miles
			expectedTotal := record.EndingMiles - record.BeginningMiles
			if record.TotalMiles == 0 {
				record.TotalMiles = expectedTotal
			} else if record.TotalMiles != expectedTotal {
				log.Printf("Warning: Total miles (%d) doesn't match calculated (%d) for vehicle %s",
					record.TotalMiles, expectedTotal, record.VehicleID)
...
func parseSchoolBusRow(row []string, reportMonth string, reportYear int) *SchoolBusRecord {
	...
	// Validate mileage data for school buses
	if record.EndingMiles > 0 && record.BeginningMiles > 0 {
		if record.EndingMiles < record.BeginningMiles {
			log.Printf("Warning: School bus %s ending miles (%d) less than beginning miles (%d)", 
				record.BusID, record.EndingMiles, record.BeginningMiles)
			record.TotalMiles = 0 // Set to 0 to indicate invalid data
		} else {
			// Calculate expected total miles
			expectedTotal := record.EndingMiles - record.BeginningMiles
			if record.TotalMiles == 0 {
				...
				log.Printf("Warning: School bus %s total miles (%d) doesn't match calculated (%d)",
					record.BusID, record.TotalMiles, expectedTotal)
				// Use calculated value
				record.TotalMiles = expectedTotal
			}
		}
	...
	log.Printf("Parsed school bus: ID=%s, Status=%s, Location=%s, Miles=%d",
		record.BusID, record.Status, record.Location, record.TotalMiles)
